...
const SingleGridItem = ({ item }: { item: Product }) => {
  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-none bg-white border-[3px] border-[var(--brutal-black)] min-h-[270px] mb-4 brutal-box group-hover:translate-x-0.5 group-hover:translate-y-0.5 group-hover:shadow-none transition-all">
        ...
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
          <button
            ...
            className="flex items-center justify-center w-9 h-9 rounded-none border-2 border-[var(--brutal-black)] text-[var(--brutal-black)] bg-white hover:bg-[var(--brutal-yellow)] brutal-btn"
          >
            <svg
          ...
          <button
            ...
            className="inline-flex font-bold text-custom-sm py-[7px] px-5 rounded-none bg-[var(--brutal-black)] text-white border-2 border-[var(--brutal-black)] brutal-btn hover:bg-[var(--brutal-yellow)] hover:text-[var(--brutal-black)]"
          >
            Add to cart
          </button>
          ...
          <button
            ...
            id="favOne"
            className={`flex items-center justify-center w-9 h-9 rounded-none border-2 border-[var(--brutal-black)] brutal-btn bg-white ${isInWishlist ? "text-red" : "text-[var(--brutal-black)]"
              } hover:bg-[var(--brutal-yellow)]`}
          >
            <svg
              className="fill-current"
              width="16"
      ...
      <div className="flex items-center gap-2.5 mb-2">
        ...
        <p className="text-custom-sm font-bold text-[var(--brutal-black)]">({item.reviews})</p>
      </div>

      <h3 className="font-bold text-[var(--brutal-black)] ease-out duration-200 hover:underline mb-1.5">
        {item.title}
      </h3>

      <span className="flex items-center gap-2 font-bold text-lg text-[var(--brutal-black)]">
        <span>₹{item.discountedPrice.toLocaleString('en-IN')}</span>
        <span className="text-[var(--brutal-black)]/60 line-through">₹{item.price.toLocaleString('en-IN')}</span>
      </span>
    </div>
  );
};
