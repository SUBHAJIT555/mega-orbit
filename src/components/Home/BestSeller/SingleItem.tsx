...
const SingleItem = ({ item }: { item: Product }) => {
  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden rounded-none bg-white border-[3px] border-[var(--brutal-black)] min-h-[403px] brutal-box group-hover:translate-x-0.5 group-hover:translate-y-0.5 group-hover:shadow-none transition-all">
        <div className="text-center px-4 py-7.5">
          ...
          <h3 className="font-black text-[var(--brutal-black)] hover:underline mb-1.5 cursor-pointer">
            {item.title}
          </h3>

          <span className="flex items-center justify-center gap-2 font-black text-lg">
            <span className="text-[var(--brutal-black)]">₹{item.discountedPrice.toLocaleString('en-IN')}</span>
            <span className="text-[var(--brutal-black)]/50 line-through">₹{item.price.toLocaleString('en-IN')}</span>
          </span>
        </div>
        ...
        <div className="absolute right-0 bottom-0 translate-x-full w-full flex flex-col gap-2 p-5.5 ease-linear duration-300 group-hover:translate-x-0">
          <button
            ...
            id="bestOne"
            className="flex items-center justify-center w-9 h-9 rounded-none border-2 border-[var(--brutal-black)] text-[var(--brutal-black)] bg-white brutal-btn hover:bg-[var(--brutal-yellow)]"
          >
            <svg
          ...
          <button
            ...
            id="addCartOne"
            className="flex items-center justify-center w-9 h-9 rounded-none border-2 border-[var(--brutal-black)] text-[var(--brutal-black)] bg-[var(--brutal-yellow)] brutal-btn"
          >
            <svg
              className="fill-current"
              width="16"
          ...
          <button
            ...
            id="addFavOne"
            className={`flex items-center justify-center w-9 h-9 rounded-none border-2 border-[var(--brutal-black)] brutal-btn ${isInWishlist ? "text-red bg-red-100" : "text-[var(--brutal-black)] bg-white hover:bg-[var(--brutal-yellow)]"}`}
          >
            <svg
              className="fill-current"
              width="16"
