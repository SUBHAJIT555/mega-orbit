...
const Header = () => {
  ...
  return (
    <header
      className="fixed left-0 top-0 w-full z-9999 bg-[var(--brutal-bg)] border-b-4 border-[var(--brutal-black)] transition-all ease-in-out duration-300"
    >
                ...
                <div className="flex items-center">
                  ...
                  <div className="relative max-w-[333px] sm:min-w-[333px] w-full">
                    ...
                    <input
                      ...
                      autoComplete="off"
                      className="custom-search w-full rounded-none bg-white border-l-0 border-2 border-[var(--brutal-black)] py-2.5 pl-4 pr-10 text-[var(--brutal-black)] placeholder:text-gray-500 outline-none focus:ring-0 focus:border-[var(--brutal-black)]"
                    />
                    ...
                    <button
                      ...
                      id="search-btn"
                      aria-label="Search"
                      className="flex items-center justify-center absolute right-3 top-1/2 -translate-y-1/2 text-[var(--brutal-black)] hover:text-[var(--brutal-cyan)]"
                    >
                      <svg
                    ...
                    {showSearchResults && searchResults.length > 0 && (
                      <div className="absolute top-full left-0 right-0 mt-1 bg-white border-2 border-[var(--brutal-black)] rounded-none z-9999 max-h-[400px] overflow-y-auto brutal-box">
                        <div className="p-2">
                          {searchResults.map((product) => (
                            <div
                              key={product.id}
                              onClick={() => handleProductClick(product.id)}
                              className="flex items-center gap-3 p-2.5 hover:bg-[var(--brutal-yellow)] cursor-pointer border-b-2 border-[var(--brutal-black)] last:border-b-0"
                            >
                              <div className="flex-shrink-0 w-12 h-12 relative rounded-none overflow-hidden border-2 border-[var(--brutal-black)]">
                                <Image src={product.img} alt={product.title} fill className="object-cover" sizes="48px" />
                              </div>
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-bold text-[var(--brutal-black)] truncate">{product.title}</p>
                                <p className="text-xs font-bold text-[var(--brutal-cyan)] truncate">₹{product.discountedPrice.toLocaleString('en-IN')}</p>
                              </div>
                            </div>
                          ))}
                        </div>
          ...
          <div className="flex w-full lg:w-auto items-center gap-7.5">
            ...
            <div className="flex w-full lg:w-auto justify-between items-center gap-5">
              <div className="flex items-center gap-5">
                ...
                >
                  <span className="inline-block relative">
                    ...
                    <span className="flex items-center justify-center font-black text-2xs absolute -right-2 -top-2.5 bg-[var(--brutal-yellow)] w-5 h-5 rounded-none border-2 border-[var(--brutal-black)] text-[var(--brutal-black)]">
                      {product.length}
                    </span>
                  </span>
                  <div>
                    <span className="block text-2xs font-bold text-[var(--brutal-black)] uppercase tracking-wider">cart</span>
                    <p className="font-black text-custom-sm text-[var(--brutal-black)]">₹{totalPrice.toLocaleString('en-IN')}</p>
                  </div>
                </button>
              </div>
              ...
              <button
                id="Toggle"
                aria-label="Toggler"
                className="xl:hidden block p-2 border-2 border-[var(--brutal-black)] bg-white hover:bg-[var(--brutal-yellow)] rounded-none brutal-btn"
                onClick={() => setNavigationOpen(!navigationOpen)}
              >
                <span className="block relative cursor-pointer w-5.5 h-5.5">
                  <span className="du-block absolute right-0 w-full h-full">
                    <span
                      className={`block relative top-0 left-0 bg-[var(--brutal-black)] rounded-none w-0 h-0.5 my-1 ease-in-out duration-200 delay-[0] ${!navigationOpen && "!w-full delay-300"}`}
                    ></span>
                    <span
                      className={`block relative top-0 left-0 bg-[var(--brutal-black)] rounded-none w-0 h-0.5 my-1 ease-in-out duration-200 delay-150 ${!navigationOpen && "!w-full delay-400"}`}
                    ></span>
                    <span
                      className={`block relative top-0 left-0 bg-[var(--brutal-black)] rounded-none w-0 h-0.5 my-1 ease-in-out duration-200 delay-200 ${!navigationOpen && "!w-full delay-500"}`}
                    ></span>
                  </span>
                  <span className="block absolute right-0 w-full h-full rotate-45">
                    <span
                      className={`block bg-[var(--brutal-black)] rounded-none ease-in-out duration-200 delay-300 absolute left-2.5 top-0 w-0.5 h-full ${!navigationOpen && "!h-0 delay-[0]"}`}
                    ></span>
                    <span
                      className={`block bg-[var(--brutal-black)] rounded-none ease-in-out duration-200 delay-400 absolute left-0 top-2.5 w-full h-0.5 ${!navigationOpen && "!h-0 dealy-200"}`}
                    ></span>
                  </span>
                </span>
              </button>
      ...
      <div className="border-t-4 border-[var(--brutal-black)]">
        <div className="max-w-[1170px] mx-auto px-4 sm:px-7.5 xl:px-0">
          <div className="flex items-center justify-between">
            <div
              className={`w-[288px] absolute right-4 top-full xl:static xl:w-auto h-0 xl:h-auto invisible xl:visible xl:flex items-center justify-between ${navigationOpen &&
                `!visible bg-white border-2 border-[var(--brutal-black)] !h-auto max-h-[400px] overflow-y-scroll rounded-none p-5 brutal-box`}`}
            >
              <nav>
                  ...
                  {menuData.map((menuItem, i) =>
                    menuItem.submenu ? (
                      ...
                      >
                        <Link
                          href={menuItem.path}
                          className={`hover:text-[var(--brutal-cyan)] text-custom-sm font-bold text-[var(--brutal-black)] flex border-b-2 border-transparent hover:border-[var(--brutal-black)] ${stickyMenu ? "xl:py-4" : "xl:py-5"}`}
                        >
                          {menuItem.title}
                        </Link>
                      </li>
            ...
            <div className="hidden xl:block">
              <ul className="flex items-center gap-5.5">
                ...
                <li className="py-4">
                  <Link
                    href="/wishlist"
                    className="flex items-center gap-1.5 font-bold text-custom-sm text-[var(--brutal-black)] hover:text-[var(--brutal-cyan)]"
                  >
                    <svg
                      className="fill-current"
                      width="16"
